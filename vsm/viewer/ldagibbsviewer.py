...
class LDAGibbsViewer(object):
    ...
    def doc_isomap(self, doc=None, top=None, thres=0.4, n_neighbors=5, scale=True, trim=20): 
        """
        Take document `doc` or topic `top` and plots an isomap for the documents 
        similar/relevant to the query. 
        
        The function first determines documents to be plotted by applying `sim_doc_doc`
        or `sim_top_doc` for the given document(s) or topic(s). It then calculates 
        the similarity matrix for the list of the obtained documents using `simmat_docs`.
        Finally, a plot is generated by using the isomap algorithm in `sklearn` 
        package.

        For isomap, see Tenenbaum, J.B., De Silva, V., & Langford, J.C. Science 290(5500)
        ...
        thres : float
            Threshhold t. If t<1, documents with similarity value >t are selected.
            Otherwise, the t' most similar documents are selected where t' is the 
            the smallest integer greater than t. Devault is 0.4.
        n_neighbors : int
