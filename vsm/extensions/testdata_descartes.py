"""
This file contains convenience functions that prepare a corpus, model,
and viewer with Project Gutenberg's The Principles of Philosophy,
by Rene Descartes. text starts from line 365, code starts from line 985
roughly.
...
def create_toycorpus():
    ...
    metadata = ['document_' + str(i) for i in xrange(0, 52)]
...
def corpus_no_stoplist():
    """
    Returns a Corpus with only words with freq<=1 filtered.
    This is used for Beagle models.
    """
    from vsm.corpus.util.corpusbuilders import toy_corpus

    metadata = ['document_' + str(i) for i in xrange(0, 52)]
    c = toy_corpus(descartes, nltk_stop=False, stop_freq=1, metadata=metadata)
...
def lda(c, K=100):
    ...
    Returns an LDA model with descartes text.
    """
    from vsm.model.ldagibbs import LDAGibbs
    
    m = LDAGibbs(c, 'document', K=K)
    m.train(itr=100)
...
def tf(c):
    ...
    Returns Tf model with descartes text. 
    """
    from vsm.model.tf import TfSeq

    m = TfSeq(corpus=c, context_type='document')
    m.train()
...
def beagleenvironment(c):
    ...
    m = BeagleEnvironment(c, context_type='document')
    m.train()
