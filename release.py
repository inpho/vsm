...
### Appveyor checks ###
# check for a no-Appveyor flag
if sys.argv[-1] != '--no-travis':
    try:
        appveyor_auth = open('.travis.key').read().strip()
    except IOError as e:
        if e.errno == 2:
            print ".appveyor.key file required to hold AppVeyor API Token."
            url = "https://ci.appveyor.com/api-token"
            print url + '\n'
            sys.exit(1)
        else:
            raise e
    ...
    url += "/api/projects/inpho/vsm/history?commitId="
    url += current_commit 
