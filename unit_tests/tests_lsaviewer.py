...
from vsm.viewer.lsaviewer import *
...
class TestLsaViewer(unittest.TestCase):
    ...
    def test_Lsaviewer(self):
        ...
        sww = self.v.sim_word_word('1')
        swwl = self.v.sim_word_word(['1', '0'])
        sdd = self.v.sim_doc_doc(1)
        sddl = self.v.sim_doc_doc([1, 0])
        ...
        sw = self.v.simmat_words(['2','4','5'])
        sd = self.v.simmat_docs([1, 0])
